...
class PickandPutEnv(gym.Env):
    ...
    def compute_reward(self):
        ...
        global_put_place = self.global_put_place
        local_put_place = self.local_put_place
        
        for i in range(4):
            if self.stack_map[i] == 1:
                continue

            global_pos_j = global_put_place[i]  # objective position
            local_pos_j = local_put_place[i]
            ...
            global_pos_stack = np.linalg.norm(pos_i[:2] - global_pos_j[:2], axis=-1) < 0.4
            local_pos_stack = np.linalg.norm(pos_i[:2] - local_pos_j[:2], axis=-1) < 0.4
            global_height_stack = pos_i[..., 2] - global_pos_j[..., 2] < 0.5
            local_height_stack = pos_i[..., 2] - local_pos_j[..., 2] < 0.5

            stack = ((global_pos_stack & global_height_stack) | (local_pos_stack & local_height_stack)) & (not attach_i)  # & (not attach_j)
            # print(f"[{i}: {stack}] pos_i: {pos_i[:2]}, target: {pos_j}, attach_i: {attach_i}")
            ...
            if stack:
                if self.rearrangment:
                    ...
                    # self.stack_map[counter] = 1
                    if global_pos_stack:
                        print(f"global optima!", flush=True)
                        reward = reward + 1
                    elif local_pos_stack:
                        print(f"local optima!", flush=True)
                        reward = reward + 0.5
                    self.stack_map[i] = 1
    ...
    def reset(self, seed=None):
        ...
        self.global_put_place = np.array([[0.5, 0.8, z], [0.5, 0.8, z], [0.5, 0.8, z], [0.5, 0.8, z]])
        self.local_put_place = np.array([[0.5, -0.8, z], [0.5, -0.8, z], [0.5, -0.8, z], [0.5, -0.8, z]])
