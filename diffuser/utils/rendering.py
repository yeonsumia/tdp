...
def norm_obs(env_name, observations):
...
class MazeRenderer:
    ...
    def renders(self, observations, start, end, conditions=None, title=None):
        plt.clf()
        fig = plt.gcf()
        fig.set_size_inches(5, 4.74)
        plt.imshow(self._background * .9,
            extent=self._extent, cmap=plt.cm.binary, vmin=0, vmax=1)
    
        plt.scatter(start[1], start[0], s=400, marker='o', edgecolors='black', color='white', alpha=1, linewidth=3, zorder=50)
        plt.scatter(start[1], start[0], s=80, marker='o', edgecolors='black', color='black', alpha=1, linewidth=3, zorder=50)

        # goal
        plt.scatter(end[1], end[0], marker='o', s=400, edgecolors='black', color='white', alpha=1, linewidth=3, zorder=50)
        plt.scatter(end[1], end[0], marker='*', s=240, edgecolors='black', color='black', alpha=1, zorder=50)

        path_length = len(observations)
        # colors = plt.cm.jet(np.linspace(0,1,path_length))
        colors = plt.cm.Reds(np.linspace(1.0, 0.2, path_length))
        plt.plot(observations[:,1], observations[:,0], c='black', zorder=10)
        plt.scatter(observations[:,1], observations[:,0], c=colors, zorder=20, s=60)
        plt.axis('off')
        plt.title(title)
        img = plot2img(fig, remove_margins=self._remove_margins)
        return img
    
    def composite(self, savepath, paths, start, end, ncol=5, **kwargs):
        '''
            savepath : str
        ...
        for path, kw in zipkw(paths, **kwargs):
            img = self.renders(*path, start, end, **kw)
            images.append(img)
...
class Maze2dRenderer(MazeRenderer):
    ...
    def renders(self, observations, start, end, conditions=None, **kwargs):
        ...
        start = norm_obs(self.env_name, start[None,:])[0]
        end = norm_obs(self.env_name, end[None,:])[0]
        
        if conditions is not None:
            conditions /= scale
        return super().renders(observations, start, end, conditions, **kwargs)
...
#         path_length = len(observations)
#         # colors = plt.cm.jet(np.linspace(0,1,path_length))
#         colors = plt.cm.Reds(np.linspace(0, 1, path_length))
#         plt.plot(observations[:,1], observations[:,0], c='black', zorder=10)
...
#             path_length = len(observations)
#             colors = plt.cm.Reds(np.linspace(0.5, 0.9, path_length))
#             plt.plot(observations[:,1], observations[:,0], c='r', zorder=10)
#             # plt.scatter(observations[:,1], observations[:,0], c=colors, zorder=20, alpha=0.5)
#             plt.axis('off')


#         images.append(plot2img(fig, remove_margins=self._remove_margins))
#         images = np.stack(images, axis=0)
