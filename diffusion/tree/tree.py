...
class TrajAggTree(object):
    ...
    def __init__(self, tree_lambda, traj_dim, action_dim=None, one_minus_alpha=0.005, start_state=None):
        ...
        state = {'state': start_state, 'step': 0, 'weight': 1, 'value': 1}
        self._root = TreeNode(None, state, self._tree_lambda)
        self.best_traj_leaf = None


    def integrate_single_traj(self, traj, length, history_length, value=1, node=None, sub_plan_timestep=None, sub_traj=None, sub_value=None, sub_step=0, is_sub_best=False):
        '''
        ...
        if sub_plan_timestep is not None:
            ...
            for z in range(len(sub_plan_timestep)):
                ...
                n = self.integrate_single_traj(sub_traj[z], length, sub_plan_timestep[z], value=sub_value[z], node=node_i[z], sub_step=sub_plan_timestep[z])

        return n if is_sub_best else node
        

    def integrate_trajectories(self, trajectories, guide=None, history_length=1, sub_guide=None, sub_plan_timestep=None, sub_trajectories=None, is_sub_best=False, best_idx=-1):
        '''
        ...
        for i in range(batch_size):
            ...
            if guide is not None:
                if sub_guide is not None:
                    sub_plan_timestep_lst = [sub_plan_timestep[i]]
                    sub_val_list = [sub_guide[i]]
                    sub_traj_list = [sub_trajectories[i]]
                    node = self.integrate_single_traj(trajectories[i], length, history_length, value=guide[i], sub_value=sub_val_list, sub_plan_timestep=sub_plan_timestep_lst, sub_traj=sub_traj_list, is_sub_best=is_sub_best)
                    if i == best_idx:
                        self.best_traj_leaf = node
                else:
                    node = self.integrate_single_traj(trajectories[i], length, history_length, value=guide[i])
            else:
    ...
    def get_next_state_original(self,):
        '''
        Acting: select the most impactful node, which has highest weight among the child nodes.
        '''
        selected_key, node = max(self._root._children.items(), key=lambda node: node[1].get_value_original())
        visit_time = len(node._states)
        max_depth = np.array(node._steps).max()
        # if len(self._root._children.items()) > 1:
            # print(f"fork: {node.node_state} (depth: {max_depth})")
