...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def guided_p_sample(self, guide, x, mask, t, clip_denoised=True, repeat_noise=False, cond_idx=0, stack_idx=0, place_idx=0, pg=False, pg_scale=1.0, guide_step=1):
        b, *_, device = *x.shape, x.device
        ...
        with torch.enable_grad():
            ...
            dist = -100. * torch.abs(stack_xy - place_xy).mean(dim=-1).mean(dim=-1)
            ...
            # if t[0] < 100:
            #     score = (y[..., 64:, cond_idx].sum()+dist)
            #     print(f"best idx: {score[torch.argmax(score)]}")
        ...
        if pg:
            # grad_phi = 2 * weights * diff / h_t * sigma.unsqueeze(1) * 2.
            # grad_phi = 2 * weights * diff / h_t * sigma.unsqueeze(1) * 1.
            # print(pg_scale)
    ...
    @torch.no_grad()
    def guided_p_sample_loop(self, guide, x, mask, cond_idx, stack_idx, place_idx, pg=False, pg_scale=1.0, guide_step=1):
        device = self.betas.device
        ...
        for i in reversed(range(0, self.num_timesteps)):
            for j in range(guide_step):
                img, values = self.guided_p_sample(guide, img, mask, torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, place_idx=place_idx, pg=pg, pg_scale=pg_scale, guide_step=guide_step)
                # img = self.p_sample(img, mask, torch.full((b,), i, device=device, dtype=torch.long))
                img[mask.bool()] = x[mask.bool()].clone()
    ...
    @torch.no_grad()
    def guided_conditional_sample(self, guide, batch_size, conditions, cond_idx, stack_idx, place_idx, pg=False, pg_scale=1.0, guide_step=1):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        ...
        for t, cond, val in conditions:
            x[:,t,:] = val
            mask[:, t] = 1
        return self.guided_p_sample_loop(guide, x, mask, cond_idx, stack_idx, place_idx, pg=pg, pg_scale=pg_scale, guide_step=guide_step)

    @torch.no_grad()
    def fast_guided_conditional_sample(self, guide, batch_size, conditions, samples, cond_idx, stack_idx, put_place, timestep):
        '''
            conditions : [ (time, state), ... ]
        '''
        # samples[:, :, :-1] = samples[:, :, 1:]
        ...
        img = x
        # progress = utils.Progress(self.num_timesteps)
        print(f"sample: {img.shape}")
        b = shape[0]
        for i in tqdm(reversed(range(0, timestep+1))):
            img, values = self.guided_p_sample(guide, img, mask[..., 0], torch.full((b,), i, device=device, dtype=torch.long), cond_idx, stack_idx, put_place)
            img[mask.bool()] = x[mask.bool()].clone()
