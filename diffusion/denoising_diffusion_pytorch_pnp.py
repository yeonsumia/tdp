...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def guided_p_sample(self, guide, x, mask, t, clip_denoised=True, repeat_noise=False, cond_idx=0, stack_idx=0, put_place=[0., 0.], pg=False, pg_scale=1.0, guide_step=1):
        b, *_, device = *x.shape, x.device
    ...
    @torch.no_grad()
    def guided_p_sample_loop(self, guide, x, mask, cond_idx, stack_idx, put_place, guide_step=1, pg=False, pg_scale=1.0):
        device = self.betas.device
        ...
        for i in reversed(range(0, self.num_timesteps)):
            for j in range(guide_step):
                img, values = self.guided_p_sample(guide, img, mask, torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, put_place=put_place, pg=pg, pg_scale=pg_scale, guide_step=guide_step)
    ...
    @torch.no_grad()
    def guided_conditional_sample(self, guide, batch_size, conditions, cond_idx, stack_idx, put_place, pg=False, pg_scale=1.0, guide_step=1):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        ...
        mask[:, -1] = 1
        # print("Set well")
        return self.guided_p_sample_loop(guide, x, mask, cond_idx, stack_idx, put_place, pg=pg, pg_scale=pg_scale, guide_step=guide_step)
    ...
    def fast_guided_conditional_sample(self, guide, batch_size, conditions, samples, cond_idx, stack_idx, put_place, timestep):
        ...
        for i in reversed(range(0, timestep+1)):
            img, values = self.guided_p_sample(guide, img, mask[..., 0], torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, put_place=put_place)
            img[mask.bool()] = x[mask.bool()].clone()
