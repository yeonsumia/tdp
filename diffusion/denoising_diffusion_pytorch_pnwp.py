...
class GaussianDiffusion(nn.Module):
    ...
    def calc_vlb(self, x_start, mask, cond_val, t=15):
        ...
        mean, _, log_variance_clipped = self.p_mean_variance(x=x_t, mask=mask, t=t, clip_denoised=True)
    ...
    def p_sample_loop(self, x, mask):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, mask[..., 0], torch.full((b,), i, device=device, dtype=torch.long))
            img[mask.bool()] = x[mask.bool()].clone()
        return img
    ...
    @torch.no_grad()
    def guided_p_sample(self, guide, x, mask, t, clip_denoised=True, repeat_noise=False, cond_idx=0, stack_idx=0, global_put_place=[0., 0.], local_put_place=[0., 0.], pg=False, pg_scale=1.0, guide_step=1):
        b, *_, device = *x.shape, x.device
        ...
        with torch.enable_grad():
            ...
            if pg:
                ...
                diff = diff[~torch.eye(diff.shape[0], device=device).bool()].reshape(diff.shape[0], -1, diff.shape[2], diff.shape[3])
                ...
                # weights = torch.exp(- (square_distance / h_t))
                grad = torch.zeros_like(x).to(device)
            else:
                # dist = -0.1 * torch.abs(stack_xy - place_xy).mean(dim=-1).mean(dim=-1)
                mid_xy = np.array([(local_put_place[0] + 3 * global_put_place[0])/4, (local_put_place[1] + 3 * global_put_place[1])/4])
                
                mid_xy = torch.from_numpy(mid_xy).reshape((1,1,2)).to(device)
                mid_xy = mid_xy.repeat(1,64,1)  # 64
                
                local_xy = torch.from_numpy(local_put_place).reshape((1,1,2)).to(device)
                local_xy = local_xy.repeat(1,64,1)  # 64
                
                global_xy = torch.from_numpy(global_put_place).reshape((1,1,2)).to(device)
                global_xy = global_xy.repeat(1,64,1)  # 64
                
                dist = -100. * (torch.abs(stack_xy - local_xy).mean(dim=-1).mean(dim=-1) -1.5 * torch.abs(stack_xy - mid_xy).mean(dim=-1).mean(dim=-1) +2 * torch.abs(stack_xy - global_xy).mean(dim=-1).mean(dim=-1))  # TODO: Important!! -100  -50
        ...
        if pg:
            grad_phi = 2 * weights * diff / h_t * sigma.unsqueeze(1) * pg_scale
            grad_phi = grad_phi.sum(dim=1)
            grad_phi = grad_phi.view_as(x[..., :7]) # [B, H, D]
        ...
        # if guide_step > 1:
        #     scalar /= guide_step
        output = model_mean + sigma * noise + sigma * grad * scalar
        if pg:
            output[..., :7] += grad_phi * scalar
        # pdb.set_trace()
        return output, 0.

    @torch.no_grad()
    def guided_p_sample_loop(self, guide, x, mask, cond_idx, stack_idx, global_put_place, local_put_place, pg=False, pg_scale=1.0, guide_step=1):
        device = self.betas.device
        ...
        for i in reversed(range(0, self.num_timesteps)):
            for j in range(guide_step):
                img, values = self.guided_p_sample(guide, img, mask, torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, global_put_place=global_put_place, local_put_place=local_put_place, pg=pg, pg_scale=pg_scale, guide_step=guide_step)
    ...
    @torch.no_grad()
    def guided_conditional_sample(self, guide, batch_size, conditions, cond_idx, stack_idx, global_put_place, local_put_place, pg=False, pg_scale=1.0, guide_step=1):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        ...
        # pdb.set_trace()
        # actual target position is not given for local setting
        # my_val[..., 7+stack_idx*8:9+stack_idx*8] = torch.from_numpy(put_place).to(device)  # only change set_block's place
        # x[:, -1, :] = my_val
        # mask[:, -1] = 1
        # print("Set well")
        return self.guided_p_sample_loop(guide, x, mask, cond_idx, stack_idx, global_put_place, local_put_place, pg=pg, pg_scale=pg_scale, guide_step=guide_step)

    @torch.no_grad()
    def fast_guided_conditional_sample(self, guide, batch_size, conditions, samples, cond_idx, stack_idx, global_put_place, local_put_place, timestep):
        '''
            conditions : [ (time, state), ... ]
        '''
        # samples[:, :, :-1] = samples[:, :, 1:]
        ...
        for i, condition in enumerate(conditions):
            for t, obs_dim, cond in condition:
                x[i,t,:] = cond
                mask[i,t,:] = 1 
        my_val = conditions[0][0][2].clone()
        # pdb.set_trace()
        # my_val[..., 7+stack_idx*8:9+stack_idx*8] = torch.from_numpy(put_place).to(device)  # only change set_block's place
        # x[:, -1, :] = my_val
        # mask[:, -1, :] = 1
        ...
        for i in reversed(range(0, timestep+1)):
            img, values = self.guided_p_sample(guide, img, mask[..., 0], torch.full((b,), i, device=device, dtype=torch.long), cond_idx=cond_idx, stack_idx=stack_idx, global_put_place=global_put_place, local_put_place=local_put_place)
            img[mask.bool()] = x[mask.bool()].clone()
