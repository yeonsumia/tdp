...
def exp_score(xy, goal, height, sig=0.5):
    # print(f"xy: {xy[0]}, goal: {goal}")
    # xy: [H // 2, 2]
    # goal: [1, 2]
    # print(f"result: {(torch.exp(-(xy - goal) ** 2  / 0.4) * height).shape}")
    exp_sc = torch.exp(-((xy - goal) ** 2).mean(dim=-1) / sig) * height
    # only in small region where (xy - goal) ** 2 < 0.1, multiply by height
    # rest of the area has no effect
    # exp_sc = torch.where(exp_sc > 2, exp_sc * height, exp_sc)
    return exp_sc

def bonus_score(xy, goal, height):
    bonus = torch.where(((xy - goal) ** 2).mean(dim=-1) < 0.1, torch.ones_like(xy[..., 0]) * height, torch.zeros_like(xy[..., 0]))
    return bonus
        
def guide_seq_match_nav(xy, visited_goals, norm_goal_lst):
    norm_1st_goal, norm_2nd_goal, norm_3rd_goal, norm_5th_goal = norm_goal_lst
    exp_score_1st_goal = exp_score(xy, norm_1st_goal.unsqueeze(0).unsqueeze(0), 4, sig=0.05)
    exp_score_2nd_goal = exp_score(xy, norm_2nd_goal.unsqueeze(0).unsqueeze(0), 2, sig=0.15)
    exp_score_3rd_goal = exp_score(xy, norm_3rd_goal.unsqueeze(0).unsqueeze(0), 0.5, sig=0.2)
    # exp_score_4th_goal = exp_score(xy, norm_4th_goal.unsqueeze(0).unsqueeze(0), 4)
    exp_score_5th_goal = exp_score(xy, norm_5th_goal.unsqueeze(0).unsqueeze(0), 0.25, sig=0.25)
            
    score = torch.zeros_like(xy[..., 0])
    
    if 1 not in visited_goals:
        score += exp_score_1st_goal
    if 2 not in visited_goals:
        score += exp_score_2nd_goal
    if 3 not in visited_goals:
        score += exp_score_3rd_goal
    # if 4 not in visited_goals:
    #     score += exp_score_4th_goal
    if 5 not in visited_goals:
        score += exp_score_5th_goal
    return score

def bonus_seq_match_nav(xy, visited_goals, norm_goal_lst):
    norm_1st_goal, norm_2nd_goal, norm_3rd_goal, norm_5th_goal = norm_goal_lst
    bonus_score_1st_goal = bonus_score(xy, norm_1st_goal.unsqueeze(0).unsqueeze(0), 10)
    bonus_score_2nd_goal = bonus_score(xy, norm_2nd_goal.unsqueeze(0).unsqueeze(0), 5)
    bonus_score_3rd_goal = bonus_score(xy, norm_3rd_goal.unsqueeze(0).unsqueeze(0), 8)
    # exp_score_4th_goal = exp_score(xy, norm_4th_goal.unsqueeze(0).unsqueeze(0), 4)
    bonus_score_5th_goal = bonus_score(xy, norm_5th_goal.unsqueeze(0).unsqueeze(0), 6)
            
    score = torch.zeros_like(xy[..., 0])
    
    if 1 not in visited_goals:
        score += bonus_score_1st_goal
    if 2 not in visited_goals:
        score += bonus_score_2nd_goal
    if 3 not in visited_goals:
        score += bonus_score_3rd_goal
    # if 4 not in visited_goals:
    #     score += exp_score_4th_goal
    if 5 not in visited_goals:
        score += bonus_score_5th_goal
    return score
...
class BaseDiffusionSDE(DiffusionModel):
    ...
    def __init__(
            ...
            fix_sub_mask: Union[list, np.ndarray, torch.Tensor] = None,  # be in the shape of `x_shape`
            # Add loss weight
        ...
        super().__init__(
            nn_diffusion, nn_condition, fix_mask, fix_sub_mask, loss_weight, classifier, grad_clip_norm,
    ...
    def classifier_guidance(
            ...
            pred=None, pg=False, guide_step=1, visited_goals=None, norm_goal_lst=None):
        """
        Guided Sampling CG:
        ...
        if self.classifier is None or (w == 0.0 and not pg):
            return pred, None
        else:
            # log_p, grad = self.classifier.gradients(xt.clone(), t, condition)
            # CG for sequence matching path task
            # pred: [B, H, D]
            h = pred.shape[1] // 2
            if pg:
                # print(xt.shape)
                ...
                diff = diff[~torch.eye(diff.shape[0], device=xt.device).bool()].reshape(diff.shape[0], -1, diff.shape[-2], diff.shape[-1])
                ...
                grad_phi = 2 * weights * diff / h_t * (sigma ** 2) * 1.0
                grad_phi = grad_phi.sum(dim=1)
                # print(f"grad_phi: {grad_phi.shape}")
                grad_phi = grad_phi.view_as(xt[:, :, :2])
            if visited_goals != None and norm_goal_lst != None:
                with torch.enable_grad():
                    pred.requires_grad_()
                    score = guide_seq_match_nav(pred[:, h:, :2], visited_goals, norm_goal_lst).mean(dim=-1)
                    # print(f"score: {max(score)}") # [B]
                    # TODO: autograd
                    grad = torch.autograd.grad([score.sum()], [pred])[0]
                    # print(pred, grad)
                    # TODO: unconditional PG
                    # if pg:
                    #     # print("NO GRAD!")
    ...
    def guided_sampling(
            ...
            requires_grad: bool = False,
            visited_goals=None, norm_goal_lst=None, pg=False):
        """
        ...
        for j in range(guide_step):
            pred, logp = self.classifier_guidance(
                xt, t, alpha, sigma, model, condition_cg, w_cg, pred, visited_goals=visited_goals, norm_goal_lst=norm_goal_lst, pg=pg, guide_step=guide_step)
            xt = pred
...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def __init__(
        ...
        super().__init__(
            nn_diffusion, nn_condition, fix_mask, fix_sub_mask, loss_weight, classifier, grad_clip_norm, ema_rate, optim_params,
    ...
    def sample(
            ...
            warm_start_forward_level: float = 0.1,
            # ------------------ others ------------------ #
            requires_grad: bool = False,
        ...
        if sub_tree_expansion:
            # print(f"fix_mask: {self.fix_sub_mask.shape}, prior: {prior.shape}, xt: {xt.shape}")
            xt = xt * (1. - self.fix_sub_mask) + prior * self.fix_sub_mask
        else:
        ...
        # ===================== Denoising Loop ========================
        # print(f"visited_goals: {visited_goals}")
        ...
        for i in reversed(loop_steps):
            ...
            pred, logp = self.guided_sampling(
                ...
                model, condition_vec_cfg, w_cfg, condition_vec_cg, w_cg, requires_grad, visited_goals=visited_goals, norm_goal_lst=norm_goal_lst, pg=pg)
        ...
        if self.classifier is not None:
            with torch.no_grad():
                ...
                h = pred.shape[1] // 2
                score = guide_seq_match_nav(xt[:, h:, :2], visited_goals, norm_goal_lst).mean(dim=-1)
                # score += bonus_seq_match_nav(xt[:, h:, :2], visited_goals, norm_goal_lst).mean(dim=-1)
...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def sample(
            ...
            # ----------- Warm-Starting -----------
            warm_start_reference: Optional[torch.Tensor] = None,
            warm_start_forward_level: float = 0.2,
            # ------------------ others ------------------ #
            requires_grad: bool = False,
            preserve_history: bool = False,
            **kwargs,
